...
# summits (altitude 9) that can be reached. Sum
# the results.
...
def calcScore(terrain, location):
   x, y = location
   # If summit is reached, create and return set
   if terrain[y][x] == 9:
      return {(x, y)}
   else:
   ...
   for y in range(len(terrain)):
      for x in range(len(terrain)):
         if terrain[y][x] == 0:
            ...
            score += len(calcScore(terrain, (x, y)))
